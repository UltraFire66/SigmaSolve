...
export default function CriarComentario({navigation}){
  ...
  const[idComentario,setIdComentario] = useState(props.idcomentario);
  ...
  useEffect(() => {
   ...
   async function handleInsert(){
    ...
    if(conteudo == ''){
      ...
      const { data, error } = await supabase
      .from('comentario')
      .insert([{ conteudotexto: conteudo, nomePdf: nomePdf || null, urlPDF: publicUrlPdf || null, conteudoimg: publicUrlImg || null,  fk_topico_idtopico: null, fk_usuario_idusuario: idUsuario, fk_comentario_idcomentario:idComentario}])   
        if (error) console.error(error)
        else{
        Alert.alert('Comentario Criado com sucesso!');
        console.log('Comentario Criado com sucesso!');
        navigation.navigate('VerComentario', {comentario: props, pesquisaNavegacao: pesquisaNavegacao,disciplina: disciplina,numComentarios: numComentarios});
      }
    }      
   }
